...
router.get('/new', async (req, res, next) => {
  try {
    const { products, laborRates } = await loadCatalog();
    const company = await getCompany();
    res.render('admin/estimate-template-form', {
      pageScript: 'page-estimate-form.js', isNew: true,
      template: { name: '', title: '', description: '', deposit_percent: 50, tax_percent: company.default_tax_percent },
      items: [], products, laborRates,
    });
  } catch (err) {
    next(err);
  }
...
async function saveItems(conn, templateId, items) {
  ...
  for (let i = 0; i < items.length; i++) {
    await conn.execute(
      ...
      [templateId, items[i].product_id, items[i].labor_rate_id, items[i].description, items[i].quantity, items[i].unit_price, i]
    );
  }
}
...
router.get('/:id/edit', async (req, res, next) => {
  try {
    ...
    const { products, laborRates } = await loadCatalog();
    res.render('admin/estimate-template-form', {
      pageScript: 'page-estimate-form.js', isNew: false, template, items, products, laborRates,
    });
  } catch (err) {
    next(err);
  }
...
router.post('/:id', async (req, res, next) => {
  ...
  try {
    ...
    await conn.beginTransaction();
    await conn.execute(
      'UPDATE estimate_templates SET name=?, title=?, description=?, deposit_percent=?, tax_percent=? WHERE id=?',
      [req.body.name, req.body.title || req.body.name, req.body.description || null,
        parseFloat(req.body.deposit_percent) || 50, req.body.tax_percent === '' ? null : parseFloat(req.body.tax_percent),
        req.params.id]
    );
