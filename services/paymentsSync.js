...
// syncs refunds. Returns { payment, totalRefunded, fullyRefunded } or null if the
// charge isn't one of ours (e.g. a ConnectedHomeLedger charge on the shared account).
async function reconcileRefunds({ chargeId, paymentIntentId }) {
  ...
  if (!payment.stripe_charge_id) {
    await db.execute('UPDATE payments SET stripe_charge_id = ? WHERE id = ?', [cid, payment.id]);
  }
  ...
  try {
    ...
    for (const r of list.data) {
      ...
      await conn.execute(
        ...
        [payment.id, r.id, r.amount / 100, r.reason || null, mapRefundStatus(r.status)]
      );
    }
    ...
    await conn.execute('UPDATE payments SET amount_refunded = ? WHERE id = ?', [total, payment.id]);
    ...
    const [[inv]] = await conn.execute('SELECT * FROM invoices WHERE id = ?', [payment.invoice_id]);
    ...
    if (inv) {
      ...
      if (newStatus !== inv.status) {
        await conn.execute('UPDATE invoices SET status = ? WHERE id = ?', [newStatus, inv.id]);
      }
    }

    await conn.commit();
    return { payment, totalRefunded: Number(total), fullyRefunded };
  } catch (err) {
    await conn.rollback();
    throw err;
  } finally {
